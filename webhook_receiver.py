...
def get_box_client(app_user=True):
    ...
    if app_user:
        ...
        try:
            ...
            logger.info('no app user exists, so the service account will be used as the box api client')
            return client
...
def get_file_id_and_hash(webhook_event, box_client):
    ...
    try:
        ...
        box_hash = None
        logger.info(f'no hash available for {filename}, likely because the link is lost before the SHARED_LINK.DELETED webhook is sent')
...
def lambda_handler(event, context):
    ...
    handled_triggers = ['SHARED_LINK.CREATED', 'SHARED_LINK.UPDATED', 'SHARED_LINK.DELETED']
    if event['body']['trigger'] in handled_triggers:
        client, webhook_key = get_box_client(app_user=True)
        ddb = boto3.resource('dynamodb', region_name='us-east-1').Table('BoxWebhookTest')
        logger.info(event)
    else:
    ...
    if validate_webhook:
        ...
        is_valid = webhook.validate_message(bytes(f"{event['body']}", 'utf-8'), event['headers'], webhook_key)
        if not is_valid:
    ...
    if event['body']['trigger'] == 'SHARED_LINK.CREATED':
        ...
        logger.info('SHARED_LINK.UPDATED was not implemented because I cannot figure out what triggers them and what the event looks like')
